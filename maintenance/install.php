<?php
	...
	$randomSalt = generateRandomString( 8 );
	...
	$db->exec("INSERT INTO users(email, realname, password, salt, admin) values(\"admin@localhost\",\"Dummy Administrator user\", \"" . sha1($randomPassword.$randomSalt) . "\", \"".$randomSalt."\", 1)");
	echo "Adding Administrator user with email: admin@localhost, password: ".$randomPassword."\n";
	$db->exec("CREATE TABLE issues(title text, description text, owner integer, assigned integer, createdate integer, updated integer, status integer)");
	$db->close();
